...
def main():
    ...
    parser.add_argument("--prompt_en", type=str, default="Describe the image.",
                        help='English prompt for the first round')
    ...
    # cuda不用quant??? 原意是quant-cpu-压缩，但实际上quant none-cpu-不压缩
    # model, model_args = AutoModel.from_pretrained(
    #     args.from_pretrained,
    #     args=argparse.Namespace(
    #         fp16=False,
    ...
    #     ))
    # 以上方法输入指令中没有--quant 8
    # if (torch.cuda.is_available() and args.quant is None) else False,
    # if (torch.cuda.is_available() and args.quant is None) else 'cpu',
    model = model.eval()
    if args.quant:
    ...
    if not args.english:
        print('欢迎使用 VisualGLM 模型，输入图像URL或本地路径读图，继续输入内容对话，clear 重新开始，stop 终止程序')
    else:
        print(
            'Welcome to VisualGLM model. Enter an image URL or local file path to load an image. Continue inputting text to engage in a conversation. Type "clear" to start over, or "stop" to end the program.')
    with torch.no_grad():
        while True:
            history = None
            cache_image = None
            ...
            while True:
                ...
                print("VisualGLM：" + response.split(sep)[-1].strip())
                image_path = None
                # cache_image = None
                # print(history)
                # for i, (old, res) in enumerate(history):
                #     print(i)
                #     print(old)
                #     print(res)
                # print(str(history))
