    ...
    def __init__(self, args, transformer=None, parallel_output=True, **kw_args):
        ...
        if args.use_lora:
            ...
            self.add_mixin("lora", LoraMixin(args.num_layers, args.lora_rank, head_first=True,
                                             num_attention_heads=args.num_attention_heads,
                                             hidden_size_per_attention_head=args.hidden_size // args.num_attention_heads,
                                             layer_range=list(range(0, 28, 14)), qlora=True), reinit=True)
        self.args = args
...
class FewShotDataset(Dataset):
    def __init__(self, path, processor, tokenizer, args):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        self.images, self.input_ids, self.labels = [], [], []
        for item in data:
            image = processor(Image.open(item['img']).convert('RGB'))
            image_token = tokenizer.encode("<img>", add_special_tokens=False)
            pad_token = [tokenizer.pad_token_id] * args.image_length
            prompt_token = tokenizer.encode("</img>问：" + item['prompt'] + "\n答：", add_special_tokens=False)
            three_ids = sum([image_token, pad_token, prompt_token], [])
            answer_ids = tokenizer.encode(text=item['answer'], add_special_tokens=False)
            if len(three_ids) > args.max_source_length - 1:
                three_ids = three_ids[: args.max_source_length - 1]
            if len(answer_ids) > args.max_target_length - 2:
                answer_ids = answer_ids[: args.max_target_length - 2]
            pre_image = len(image_token)

            input_ids = tokenizer.build_inputs_with_special_tokens(three_ids, answer_ids)
            input_ids_len = input_ids.index(tokenizer.bos_token_id)
            mask_pos = input_ids_len - 1
            mask_label = [-100] * input_ids_len + input_ids[mask_pos + 1:]

            pad_len = max_seq_length - len(input_ids)
            input_ids = input_ids + [tokenizer.pad_token_id] * pad_len
            mask_label = mask_label + [tokenizer.pad_token_id] * pad_len
            if args.ignore_pad_token_for_loss:
                mask_label = [(l if l != tokenizer.pad_token_id else -100) for l in mask_label]
...
def create_dataset_function(path, args):
    ...
    print('===================create_dataset_function加载tokenizer===================')
