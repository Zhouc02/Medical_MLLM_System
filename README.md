...
因此，建议使用更先进的医学多模态模型，XrayGLM的实际诊断效果可能并未如预期。若使用更先进的模型，请根据实际情况修改`Medical_LLM/Medical_LLM/apps.py`的`ready()`函数，该函数的作用是当Django启动时同时后台挂载模型。然后根据模型的对话API，修改`Medical_LLM/MLLM/views.py`的`talk()`函数。
### NER
