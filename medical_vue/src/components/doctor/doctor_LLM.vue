  ...
  <div>
    <el-row style='position: absolute;bottom: 0;right: 0;top: 0;left: 0;'>
      ...
      <el-col :span='7' style='height: calc(100vh - 75px);border-right: 2px solid #ccc;'>
        ...
        <el-col :span='24' style='border-bottom: 1px solid #ccc;height: calc(70vh - 75px - 80px);display: flex;justify-content: center;align-items: center'>
          <img v-if="imageURL" :src="imageURL" alt='预览图片' style='width: 400px;height: auto'>
        </el-col>
        <!--诊断报告自动显示区域-->
        <el-col :span='24' v-html='html2' style='height: 30vh;overflow: auto;padding-left: 10px;padding-top: 10px;padding-right: 10px'></el-col>
      </el-col>
      <!--中间电子病历区域-->
      <el-col :span='10' style='height: calc(100vh - 75px);border-right: 2px solid #ccc;'>
        ...
        <el-col :span='24' style='height: calc(100vh - 75px - 80px - 55px - 46px - 550px);display: flex; align-items: center;justify-content: center;'>
          <el-button v-show='!showEdit' @click='copyAndPaste' type='info'>复制结果</el-button>
  ...
  methods:{
    async autoTalkWithAI(){
      ...
      for (let i = 0; i < d.length; i++) {
        ...
        html += this.output2.slice(lastIndex, start);
        html += `<span class="highlight ${type}"><span class="value">${value}</span></span>`;
        lastIndex = end;
      }
      html += this.output2.slice(lastIndex);
      this.html2 = html;
    },
    async resolveID(){
